...
public class EchoHandler extends TextWebSocketHandler {
	...
	// 메세지 큐
	private Map<String, ConcurrentLinkedQueue<TextMessage>> messageQueue = new ConcurrentHashMap<>();
    ...
    public void afterConnectionEstablished(WebSocketSession session) throws Exception {
		...
		if(senderId!=null) {	// 로그인 값이 있는 경우만
			...
			// 사용자에 대한 큐된 메시지가 있는지 확인하고 전송
            ConcurrentLinkedQueue<TextMessage> queue = messageQueue.get(senderId);
            if (queue != null && !queue.isEmpty()) {
                while (!queue.isEmpty()) {
                    session.sendMessage(queue.poll());
                }
            }
            messageQueue.remove(senderId);
		} else {
        ...
        if (msg != null) {
            ...
            if (strs.length >= 2) {
                ...
                if (targetSession != null && targetSession.isOpen()) {
                    ...
                    // 오프라인 사용자를 위해 메시지를 큐에 저장
                    messageQueue.computeIfAbsent(target, k -> new ConcurrentLinkedQueue<>()).add(new TextMessage(msg));
                }
            } else {
