...
document.addEventListener("click", async (e) => {
  if (e.target.closest(".file-x")) {
    let uuid = e.target.closest(".file-x").dataset.uuid;
    console.log(uuid);

    try {
      // 비동기로 데이터 삭제
      let result = await removeFileToServer(uuid);
      console.log(result);

      if (result > 0) {
        e.target.closest("div").querySelector("img").remove();
        e.target.closest("div").querySelector(".file-x").remove();
      }
    } catch (error) {
      console.error(error);
    }
...
function handleFileInputChange(e, fileZoneId) {
  ...
  for (let file of files) {
    ...
    ul += `<li>`;
    if (validResult) {
      ul += `<div>업로드가 가능한 파일입니다.</div>`;
      // 이미지 표시 함수 호출
      displayImage(file);
    } else {
      ul += `<div>업로드가 불가능한 파일입니다.</div>`;
    }
    ul += `${file.name}`;
...
function displayImage(file) {
  ...
  reader.onload = function (e) {
    ...
    image.style.width = "100px"; // 이미지의 크기 조절 (원하는 크기로 수정)
...
function validateButton(event) {
  ...
  // 이미지 파일이 존재하는지 여부 확인
  const hasImages = () => {
    const oldThumbImgs = document.querySelectorAll("#oldThumbFileZone img");
    const oldDetailImgs = document.querySelectorAll("#oldDetailFileZone img");
    const newThumbImgs = document.querySelectorAll(
      "#thumbFileZone .text-bg-success"
    );
    const newDetailImgs = document.querySelectorAll(
      "#detailFileZone .text-bg-success"
    );
    return (
      (oldThumbImgs.length > 0 && oldDetailImgs.length > 0) ||
      (oldThumbImgs.length > 0 && newDetailImgs.length > 0) ||
      (newThumbImgs.length > 0 && oldDetailImgs.length > 0)
    );
  };
  ...
  // 이미지 파일이 존재하는 경우에만 required 속성 제거
  thumbFileInput.required = detailFileInput.required = !hasImages();
