document.addEventListener("DOMContentLoaded", function () {
  ...
  function fetchData() {
    $.ajax({
      type: "POST",
      url: "/board/searchMore",
      data: {
        page: currentPage,
        search: $("input[name='searchKeyword']").val(),
      },
      dataType: "json",
      success: function (data) {
        if (data.length === 0) {
          ...
          data.forEach(function (dto) {
            ...
            newResultHTML += '<a href="/board/detail?bno=' + dto.bvo.bno + '">';
            newResultHTML +=
              '<span class="info-1">등록인: ' +
              dto.bvo.writer +
              '</span> <span class="info-2">' +
              dto.bvo.searchType_a +
              "</span>";
            newResultHTML +=
              '<span class="img-wrapper"><span class="img-container">';

            // Process the combinedList data
            dto.combinedList.forEach(function (fvo) {
              if (fvo.fileCategoryNum == 1) {
                newResultHTML +=
                  '<img alt="image" src="/upload/' +
                  fvo.save_dir.replace(/\\/g, "/") +
                  "/" +
                  fvo.uuid +
                  "_" +
                  fvo.file_name +
            ...
            newResultHTML += "</span></span>";
            newResultHTML +=
              '<span class="info-3">' +
              dto.bvo.searchType_b +
              '</span> <span class="info-4">' +
              dto.bvo.title +
              "</span>";
            newResultHTML +=
              '<span class="info-5">' +
              dto.bvo.searchType_add1 +
              " " +
              dto.bvo.searchType_add2 +
              "</span>";
            newResultHTML +=
